...
# import rope
from exllamav2.rope import DeepseekV2RotaryEmbedding, DeepseekV2LinearScalingRotaryEmbedding, DeepseekV2DynamicNTKScalingRotaryEmbedding, DeepseekV2YarnRotaryEmbedding, apply_rotary_pos_emb

#import exllamav2 config
...
class ExLlamaV2DeepSeekAttention(ExLlamaV2Module):

    name: str = "DeepSeek Attention"

    layer_idx: int
    input_layernorm: ExLlamaV2RMSNorm | ExLlamaV2LayerNorm | None
    q_proj: ExLlamaV2Linear | None
    q_a_proj: ExLlamaV2Linear | None
    q_b_proj: ExLlamaV2Linear | None
    ...
    def __init__(self,
        ...
        self.rotary_embedding_base = cfg.rotary_embedding_base # rope_theta
        ...
        if self.q_lora_rank is None:
            self.q_proj = ExLlamaV2Linear(model, key + ".self_attn.q_proj", hidden_size, self.num_heads * self.q_head_dim, has_bias=False, f_key=None)
        else:
    ...
    def numel(self) -> int:
        if self.q_lora_rank is None:
            numel = self.q_proj.numel() + \
                    self.kv_a_proj_with_mqa.numel() + \
                    self.kv_a_layernorm.numel() + \
                    self.kv_b_proj.numel() + \
                    self.o_proj.numel()
        else:
            numel = self.q_a_proj.numel() + \
                    self.q_a_layernorm() + \
                    self.q_b_proj() + \
                    self.kv_a_proj_with_mqa.numel() + \
                    self.kv_a_layernorm.numel() + \
