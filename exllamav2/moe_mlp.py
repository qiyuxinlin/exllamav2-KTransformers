...
class ExLlamaV2MoEShareMLP(ExLlamaV2Module):

    name: str = "MoE MLP With Share Experts"
    ...
    def __init__(self,
        ...
        self.shared_expert_w1 = ExLlamaV2Linear(model,key+cfg.arch.mlp_share_experts_gate, hidden_size, cfg.shared_expert_intermediate_size, cfg.mlp_share_experts_bias, f_key = None, f_beg = 0)
        self.shared_expert_w2 = ExLlamaV2Linear(model,key+cfg.arch.mlp_share_experts_down, cfg.shared_expert_intermediate_size, hidden_size, cfg.mlp_share_experts_bias, f_key = None, f_beg = 0)
        self.shared_expert_w3 = ExLlamaV2Linear(model,key+cfg.arch.mlp_share_experts_up, hidden_size, cfg.shared_expert_intermediate_size, cfg.mlp_share_experts_bias, f_key = None, f_beg = 0)
        self.shared_expert_gate = ExLlamaV2Linear(model, key+cfg.arch.shared_expert_gate, in_features=cfg.hidden_size, out_features=1, has_bias=False, pad32=False, f_key=None, f_beg=0)
        self.submodules = [self.post_attention_layernorm,
                        self.gate] + \
                        self.w1 + \
                        self.w2 + \
                        self.w3 + \
                        [self.shared_expert_w1,
                         self.shared_expert_w2,
                         self.shared_expert_w3, 
    ...
    def load(self):
        ...
        self.shared_expert_w1.load()
        self.shared_expert_w2.load()
        self.shared_expert_w3.load()
        self.shared_expert_gate.load()
    ...
    def unload(self):
        ...
        self.shared_expert_w1.unload()
        self.shared_expert_w2.unload()
        self.shared_expert_w3.unload()
        self.shared_expert_gate.unload()
    ...
    def weight_footprint(self) -> int:
        ...
        return self.post_attention_layernorm.weight_footprint() + \
               ...
               sum(self.w2[e].weight_footprint() for e in range(self.num_experts)) + \
               sum(self.w3[e].weight_footprint() for e in range(self.num_experts)) + \
               self.shared_expert_w1.weight_footprint() + \
               self.shared_expert_w2.weight_footprint() + \
               self.shared_expert_w3.weight_footprint() + \
               self.shared_expert_gate.weight_footprint()
    ...
    def set_device_idx(self, idx: int):
        ...
        for e in range(self.num_experts):
            ...
            self.w2[e].set_device_idx(idx)
            self.w3[e].set_device_idx(idx)
        self.shared_expert_w1.set_device_idx(idx)
        self.shared_expert_w2.set_device_idx(idx)
        self.shared_expert_w3.set_device_idx(idx)
        self.shared_expert_gate.set_device_idx(idx)
    ...
    def forward_torch(self,
        ...
        ### Qwen2 share_experts
        if self.model.config.arch.mlp_act_func == "silu":
            shared_expert_output = self.shared_expert_w2.forward(F.silu(self.shared_expert_w1.forward(hidden_states)) * self.shared_expert_w3.forward(hidden_states))
        else:
            shared_expert_output = self.shared_expert_w2.forward(F.gelu(self.shared_expert_w1.forward(hidden_states)) * self.shared_expert_w3.forward(hidden_states))
        shared_expert_output = F.sigmoid(self.shared_expert_gate.forward(hidden_states)) * shared_expert_output
...
class ExLlamaV2DeepSeekMLP(ExLlamaV2Module):

    name: str = "DeepSeekMoE MLP"
    ...
    def forward_torch(self,
        ...
        if self.topk_method == "greedy":
            ...
            group_routing_weights = (
                routing_weights.view(batch_size * sequence_length, self.n_group, -1).max(dim=-1).values
            )  # [n, n_group]
            group_idx = torch.topk(
                group_routing_weights, k=self.topk_group, dim=-1, sorted=False
            )[
                1
            ]  # [n, top_k_group]
            group_mask = torch.zeros_like(group_routing_weights)  # [n, n_group]
            group_mask.scatter_(1, group_idx, 1)  # [n, n_group]
            score_mask = (
                group_mask.unsqueeze(-1)
                .expand(
                    batch_size * sequence_length, self.n_group, self.num_experts // self.n_group
                )
                .reshape(batch_size * sequence_length, -1)
            )  # [n, e]
            tmp_scores = routing_weights.masked_fill(~score_mask.bool(), 0.0)  # [n, e]
            routing_weights, selected_experts = torch.topk(tmp_scores, k=self.num_experts_per_token, dim=-1)

        if self.num_experts_per_token > 1 and self.model.config.norm_topk_prob:
            routing_weights /= routing_weights.sum(dim = -1, keepdim = True)
        else:
        ...
        for i, num_tokens in enumerate(tokens_per_expert):
            ...
            gate = self.w1[i].forward(tokens_for_this_expert, loras = loras)
            up = self.w3[i].forward(tokens_for_this_expert, loras = loras)
            if self.model.config.arch.mlp_act_func == "silu":
                current_hidden_states = F.silu(gate)
            elif self.model.config.arch.mlp_act_func == "gelu":
                current_hidden_states = F.gelu(gate)
            current_hidden_states *= up
            if intermediates: result[f"pre_down.{i}"] = current_hidden_states
            current_hidden_states = self.w2[i].forward(current_hidden_states, loras = loras)
        ...
        ### DeepSeek share_experts
        if self.model.config.arch.mlp_act_func == "silu":
            shared_expert_output = self.shared_expert_w2.forward(F.silu(self.shared_expert_w1.forward(hidden_states)) * self.shared_expert_w3.forward(hidden_states))
        else:
            shared_expert_output = self.shared_expert_w2.forward(F.gelu(self.shared_expert_w1.forward(hidden_states)) * self.shared_expert_w3.forward(hidden_states))
        final_out = final_out + shared_expert_output
        ###

        final_out = final_out.reshape(batch_size, sequence_length, hidden_dim)
        final_out += residual

        if intermediates:
            result["hidden_states"] = final_out
            return result
        else:
            return final_out
#####
